...
@if(in_array('9-2',$userMenu) or Auth::user()->is_superuser == 1)
@isset($edit)
<form method="post" action="{{ route('update_staff', $edit->id) }}" enctype="multipart/form-data">
    @csrf
    <div class="card">
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <div class="form-group">
                    <label for="position_id">Positions:</label><br>
                    <select class="form-control" id="position_id" name="position_id">
                        <option value="">Choose position</option>
                        @foreach($posdata as $p)
                            <option value="{{ $p->posid }}" {{ $edit->position_id == $p->posid ? 'selected' : '' }}>{{ $p->department }}/{{ $p->position }}</option>
                        @endforeach
                    </select><br>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="salary" name="salary" value="{{ $edit->salary }}"><br>
                </div>
                <div class="form-group">
                    <label for="work">Work:</label><br>
                    <input type="date" class="form-control" id="work" name="work" value="{{ $edit->work }}"><br>
                </div>
                <button type="submit" class="btn btn-primary" title="Update staff">Update</button> <a href="{{ route('staff', ['cancel_clicked' => 1]) }}"><button type="button" title="Cancel" class="btn btn-secondary mr-2">Cancel</button></a>
            </form>
        </div>
    </div>
</form>
@else
<form method="post" action="{{ route('submit_staff') }}" enctype="multipart/form-data">
    @csrf
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">Staff</h4>
            ...
            <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="position_id" name="position_id">
                        <option value="">Choose position</option>
                        @foreach($posdata as $p)
                            <option value="{{ $p->id }}">{{ $p->department }}/{{ $p->position }}</option>
                        @endforeach
                    </select><br>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="work">Work:</label><br>
                    <input type="date" class="form-control" id="work" name="work"><br>
                </div>
                <button type="submit" class="btn btn-primary" title="Send staff">Submit</button>
            </form>
        </div>
    </div>
</form>
...
<div class="card-body">
    ...
    <form action = "{{ route('delete_selected_staff') }}" >
    <table class="table table-dark">
        <thead>
            ...
            @endif
                ...
                @if(in_array('9-2',$userMenu) or Auth::user()->is_superuser == 1)
                <th> Actions
                <button class="btn btn-danger" title="Delete selected staff"><i class="mdi mdi-delete"></i></button>
                <a href="{{route('export_staff')}}" class="btn btn-success" title="Export Staff">
        ...
        @foreach($data as $i=>$info)
            <tr>
            @if(in_array('9-2',$userMenu) or Auth::user()->is_superuser == 1)
                <td><input type="checkbox" name="secim[]" value="{{ $info->sid }}"></td>
                @endif
                <td>{{$i+=1}} </td>
                ...
                @if(in_array('9-2',$userMenu) or Auth::user()->is_superuser == 1)
                <td>
                    <a href="{{route('delete_staff',$info->sid)}}"><button type="button" title="Delete staff" class="btn btn-danger btn-rounded btn-fw"><i class="mdi mdi-delete"></i></button></a>
                    <a href="{{route('edit_staff',$info->sid)}}"><button type="button" title="Edit staff" class="btn btn-warning btn-rounded btn-fw"><i class="mdi mdi-grease-pencil"></i></button></a>
                </td>
                @endif
            </tr>
            @endforeach
